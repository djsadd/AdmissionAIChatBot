...
async def ask_ai(uuid, query, platform="Telegram"):
    session_id = uuid
    categories = await categorize_query(query)
        ...
        if response.strip():
            try:
                ...
                if parsed_response.get("answer") == "Нет ответа":
                    print("Пустой или нейтральный ответ.")
...
async def categorize_query(query):
    response = await ask_groq_category(query)
    if response.strip():
        try:
            parsed_response = json.loads(response)
